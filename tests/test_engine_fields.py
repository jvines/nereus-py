...
def test_default_engine_carries_no_values():
    """`engines.DEFAULT` documents the fixed-dim default; it does not set it.

    It used to hard-code n_temps/n_walkers/n_steps/n_burnin/init_strategy.
    Three of those merely restated Julia's defaults and the budget drifted:
    Nereus v0.6.0 raised pt_emcee to 3000/2000 and this object went on
    overriding it back down to 1500/750, which fails R-hat.
    """
    wire = engines.DEFAULT.to_wire()
    assert wire["engine"] == "pt_emcee"
    assert wire["options"] == {}
