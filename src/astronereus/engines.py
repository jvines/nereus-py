...
Trans-dim: `td` is accepted by pt/moms/daedalus/rjmcmc/transdim_pt_emcee
...
class PTEmcee(Engine):
    ...
    thin: int | None = None
    seed: int | None = None
    pf_n_runs: int | None = None
    convergence_stop: bool | None = None
    betas: Sequence[float] | None = None
    beta_min: float | None = None
    adapt_ladder: bool | None = None
    ladder_adapt_window: int | None = None
    ladder_adapt_ν0: float | None = None
    ...
    laplace_switch_tol: float | None = None
    bridge_headline: bool | None = None
    bridge_n: int | None = None
    bridge_warn_tol: float | None = None
    untemper_transit: bool | None = None
    # Nereus >= 0.6.0. Burn-in only: walkers stranded on a region whose total
    # posterior mass is bounded below e^-10 of the mode's are moved onto it.
    # On by default in Julia; `False` restores the pre-0.6.0 behaviour.
    prune_stranded: bool | None = None
    # Nereus >= 0.6.1. Probability per walker and step of proposing the mirror
    # (Ω, ω) -> (Ω+π, ω+π) of an astrometry-only planet, accepted with the
    # tempered M-H ratio. Astrometry alone cannot tell the two apart, and the
    # stretch move cannot cross between them: Gaia-4 came back 61/39 where the
    # symmetry demands 50/50, and R-hat saw nothing. 0.1 in Julia; `0`
    # restores the pre-0.6.1 behaviour. Never offered to a planet with RV or
    # transit data, so it costs nothing on those fits.
    node_flip: float | None = None
    show_progress: bool | None = None
...
# What you get when you pass no `engine=`. NOTHING IS SENT: `_fit._run` omits
# the engine entirely, and Julia picks by SHAPE -- pt_emcee for a fixed-dim
# fit, transdim_pt_emcee when a `transdim` block is present (src/api.jl:697,
# src/runner.jl:1321). This object only documents the fixed-dim half of that,
# so `engines.DEFAULT` still names something you can inspect and copy.
#
# It carries no values for the same reason nothing else in this file does.
# Until 0.4.23 it hard-coded n_temps=16, n_walkers=100, n_steps=1500,
# n_burnin=750, init_strategy="prior" -- a "quick exploratory" half-budget.
# Three of those merely restated Julia's own defaults, and the budget DRIFTED:
# Nereus v0.6.0 raised pt_emcee to 3000/2000 because 2000/1000 left 1 run in 8
# non-convergent, and this line went on overriding it back down to 1500/750,
# which fails R-hat (1.33-1.77) on the very target that release fixed. It is
# called out by name in the v0.6.0 release notes.
#
# Sending nothing also fixes trans-dim, which was broken outright: an explicit
# engine name makes api.jl attach `td` to that sampler's options, and
# `run_engine` rejects options the sampler does not declare -- so every
# `fit_*(..., transdim=...)` from Python died with "engine pt_emcee does not
# accept td" before sampling. Asking for trans-dim is now enough, as it is
# from Julia.
#
# pt_emcee, not pt, remains the right fixed-dim default: on the repo's own
# astrometry validation set pt is 14-20s faster and gets the answer wrong on
# one of the two systems (HD 159062: e=0.359 against a true 0.11), while
# pt_emcee recovers both. See test/astrometry/menu_HD159062_results.txt and
# menu_HD4747_results.txt. pt is also silent when it fails -- no swap
# acceptance, no R-hat -- whereas pt_emcee says when it is struggling. The
# trans-dim counterpart is transdim_pt_emcee for the same reasons; pt's
# coordinate-wise slice explorer has the same axis-aligned limitation there,
# with birth/death moves layered on top.
DEFAULT = PTEmcee()
